report 60002 "Sales - Invoice Preview"
{
    Caption = 'Sales - Invoice Preview';
    DefaultLayout = RDLC;
    EnableHyperlinks = true;

    PreviewMode = PrintLayout;
    // RDLCLayout = './60002SalesInvoicePreview.rdl';
    ...
    dataset
    {
        dataitem(Header; "Sales Header")
        {
            ...
            RequestFilterHeading = 'Sales Invoice Preview';
            ...
            column(CompanyAddress3; CompanyInfo."Address 2")
            {
            }
            column(CompanyAddress4; CompanyInfo.City)
            {
            }
            column(CompanyAddress5; CompanyAddr[5])
            {
            }
            column(CompanyAddress6; CompanyCountryRegion.Name)
            {
            }
            column(CompanyAddress7; CompanyInfo."Post Code")
            {
            }
            ...
            column(CompanyBankAccountCurrency; CompanyBankAccount."Currency Code")
            {
            }
            column(CompanyBankAccountCurrency_Lbl; CompanyBankAccount.FieldCaption("Currency Code"))
            {
            }
            ...
            column(CustomerAddress1; CustomerInfo.Name)
            {
            }
            column(CustomerAddress2; CustomerInfo.Address)
            {
            }
            column(CustomerAddress3; CustomerInfo."Address 2")
            {
            }
            column(CustomerAddress4; CustAddr[4])
            {
            }
            column(CustomerAddress5; CustomerInfo.City)
            {
            }
            column(CustomerAddress6; CustomerCountryRegion.Name)
            {
            }
            column(CustomerAddress7; CustomerInfo."Post Code")
            {
            }
            ...
            column(OrderNo_Lbl; FieldCaption("No."))
            {
            }
            ...
            column(NetInvoice; Format(NetInvoice, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, Header."Currency Code")))
            {
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                {
                    AutoFormatExpression = GetCurrencyCode(Header, Line);
                    AutoFormatType = 1;
                }
                ...
                begin
                    ...
                    //Start YANA Added AdvancePayment Calculation
                    if NOT L_Currency.get(Header."Currency Code") then
                        RoundingPrecision := 0.01
                    else
                        RoundingPrecision := L_Currency."Amount Rounding Precision";
                    //End YANA Added AdvancePayment Calculation
            ...
            begin
                ...
                RemainingAmount := GetRemainingAmount(Header);
                ...
                // if ("No." = '') and "Prepayment Invoice" then
                //     "No." := "Prepayment Order No.";
            end;
    ...
    begin
        ...
        if CompanyInfo."Country/Region Code" <> '' then begin
            CompanyCountryRegion.Get(CompanyInfo."Country/Region Code");
        end;
        SalesSetup.Get();
    ...
    procedure GetCurrencyCode(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"): Code[10]
    var
        SalesHeaderNo: Code[20];
    begin
        GetHeader(SalesHeader, SalesLine);
        exit(SalesHeader."Currency Code");
    end;

    local procedure GetHeader(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"): Code[20]
    var
        Currency: Record Currency;
    begin
        if SalesHeader."No." = SalesLine."Document No." then
            exit;
        if not SalesHeader.Get(SalesLine."Document No.") then
            SalesHeader.Init();
    ...
    PROCEDURE GetRemainingAmount(SalesHeader: Record "Sales Header"): Decimal;
    VAR
        CustLedgerEntry: Record "Cust. Ledger Entry";
    BEGIN
        CustLedgerEntry.SETRANGE("Customer No.", SalesHeader."Bill-to Customer No.");
        CustLedgerEntry.SETRANGE("Posting Date", SalesHeader."Posting Date");
    ...
    var
        ...
        SellerCRNumber: Text;
        SellerOtherSellerID: Text;
        SellerPhone: Text;
        CustomerName: Text;
        CustomerBuildingNo: Text;
        ...
        CustomerCRNumber: Text;
        CustomerOtherSellerID: Text;
        CustomerPhone: Text;
    ...
    begin
        ...
        if LineFeeNoteOnReportHist.FindSet() then begin
            ...
            LineFeeNoteOnReportHist.SetRange("Language Code", G_Language.GetUserLanguageCode);
    ...
    begin
        ...
        FillNameValueTable(LeftHeader, Header.FieldCaption("No."), Header."No.");
